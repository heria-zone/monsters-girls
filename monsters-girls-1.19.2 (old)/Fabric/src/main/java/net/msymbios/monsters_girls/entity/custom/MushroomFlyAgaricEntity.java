...
public class MushroomFlyAgaricEntity extends InternalEntity implements IAnimatable {
        ...
        if (entity.isOnGround()) {
            ...
            if (world.isSpaceEmpty(new Box(blockPos))) {
                if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.GRASS_BLOCK) {
                    if (Math.random() < 5e-7) world.setBlockState(blockPos, ModBlocks.HUGE_FLY_RED_AGARIC.getDefaultState(), 3);
