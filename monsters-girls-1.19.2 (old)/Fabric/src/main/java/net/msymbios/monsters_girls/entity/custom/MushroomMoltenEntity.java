...
public class MushroomMoltenEntity extends InternalEntity implements IAnimatable {
    ...
    protected void handlePlanting (WorldAccess world, double x, double y, double z, Entity entity) {
        ...
        if (entity.isOnGround()) {
            ...
            if (world.isSpaceEmpty(new Box(blockPos))) {
                var canBlock = (world.getBlockState(new BlockPos(x, y - 1, z))).getBlock();
                if (canBlock == Blocks.BASALT || canBlock == Blocks.BLACKSTONE || canBlock == Blocks.MAGMA_BLOCK) {
                    if (Math.random() < 5e-7) world.setBlockState(blockPos, ModBlocks.HUGE_MOLTEN_FUNGUS.getDefaultState(), 3);
                    if (Math.random() < 0.0005) world.setBlockState(blockPos, ModBlocks.MOLTEN_FUNGUS.getDefaultState(), 3);
                }
            }
        }
    } // handlePlanting ()
