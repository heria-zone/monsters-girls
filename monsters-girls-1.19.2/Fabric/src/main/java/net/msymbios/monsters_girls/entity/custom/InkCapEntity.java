...
public class InkCapEntity extends InternalEntity implements IAnimatable {
    ...
    public static DefaultAttributeContainer.Builder setAttributes() {
        return MobEntity.createMobAttributes()
                .add(EntityAttributes.GENERIC_MAX_HEALTH, InternalMetric.getAttribute(EntityVariant.InkCap, EntityAttribute.MAX_HEALTH))
                .add(EntityAttributes.GENERIC_ATTACK_DAMAGE, InternalMetric.getAttribute(EntityVariant.InkCap, EntityAttribute.ATTACK_DAMAGE))
                .add(EntityAttributes.GENERIC_ATTACK_SPEED, InternalMetric.getAttribute(EntityVariant.InkCap, EntityAttribute.ATTACK_SPEED))
                .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, InternalMetric.getAttribute(EntityVariant.InkCap, EntityAttribute.MOVEMENT_SPEED))
                .add(EntityAttributes.GENERIC_ARMOR, InternalMetric.getAttribute(EntityVariant.InkCap, EntityAttribute.ARMOR))
                .add(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, InternalMetric.getAttribute(EntityVariant.InkCap, EntityAttribute.ARMOR_TOUGHNESS));
    } // setAttributes ()

    // -- MODEL --
    @Override
    public Identifier getCurrentModelByID(int value) { return InternalMetric.getModel(EntityVariant.InkCap, EntityModel.byId(value)); } // getCurrentModelByID ()

    // ANIMATOR
    @Override
    public Identifier getAnimatorByID(int value) { return InternalMetric.getAnimator(EntityVariant.InkCap, EntityModel.byId(value)); } // getAnimatorByID ()

    // TEXTURE
    @Override
    public Identifier getTextureByID(int value) { return InternalMetric.getTexture(EntityVariant.InkCap, EntityTexture.byId(value)); } // getTextureByID ()

    // VARIANT
    @Override
    public String getVariant() {
        return this.getVariant(EntityVariant.InkCap.getName());
    } // getVariant ()

    // STATS
    public float getAttributeRaw(EntityAttribute attribute) {
        return InternalMetric.getAttribute(EntityVariant.InkCap, attribute);
    } // getAttributeRaw ()

    // -- Constructor --
    public InkCapEntity(EntityType<? extends TameableEntity> entityType, World world) {
        super(entityType, world);
    } // Constructor InkCapEntity ()
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, EntityData entityData, NbtCompound entityNbt) {
        this.setVariant(EntityVariant.InkCap.getName());
        this.setVariant(EntityModel.Fungus.getName());
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(VARIANT, EntityVariant.InkCap.getName());
        this.dataTracker.startTracking(MODEL_ID, EntityModel.Fungus.getId());
