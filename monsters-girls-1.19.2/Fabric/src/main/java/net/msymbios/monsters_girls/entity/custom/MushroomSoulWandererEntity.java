    ...
    protected void handlePlanting (WorldAccess world, double x, double y, double z, Entity entity) {
        ...
        if (entity.isOnGround()) {
            ...
            if (world.isSpaceEmpty(new Box(blockPos))) {
                if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.SOUL_SAND || (world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.SOUL_SOIL) {
                    if (Math.random() < 5e-7) world.setBlockState(blockPos, ModBlocks.HUGE_SOUL_WANDERER.getDefaultState(), 3);
                    if (Math.random() < 0.0005) world.setBlockState(blockPos, ModBlocks.SOUL_WANDERER_FUNGUS.getDefaultState(), 3);
                }
            }
        }
    } // handlePlanting ()
