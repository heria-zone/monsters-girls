...
public class MushroomGirlEnderPuffballEntity extends InternalEntity implements IAnimatable {
    ...
    public static DefaultAttributeContainer.Builder setAttributes() {
        return MobEntity.createMobAttributes()
                .add(EntityAttributes.GENERIC_MAX_HEALTH, InternalMetric.getAttribute(EntityVariant.EnderPuffball, EntityAttribute.MAX_HEALTH))
                .add(EntityAttributes.GENERIC_ATTACK_DAMAGE, InternalMetric.getAttribute(EntityVariant.EnderPuffball, EntityAttribute.ATTACK_DAMAGE))
                .add(EntityAttributes.GENERIC_ATTACK_SPEED, InternalMetric.getAttribute(EntityVariant.EnderPuffball, EntityAttribute.ATTACK_SPEED))
                .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, InternalMetric.getAttribute(EntityVariant.EnderPuffball, EntityAttribute.MOVEMENT_SPEED))
                .add(EntityAttributes.GENERIC_ARMOR, InternalMetric.getAttribute(EntityVariant.EnderPuffball, EntityAttribute.ARMOR))
                .add(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, InternalMetric.getAttribute(EntityVariant.EnderPuffball, EntityAttribute.ARMOR_TOUGHNESS));
    } // setAttributes ()

    // -- MODEL --
    @Override
    public Identifier getCurrentModelByID(int value) { return InternalMetric.getModel(EntityVariant.EnderPuffball, EntityModel.byId(value)); } // getCurrentModelByID ()

    // ANIMATOR
    @Override
    public Identifier getAnimatorByID(int value) { return InternalMetric.getAnimator(EntityVariant.EnderPuffball, EntityModel.byId(value)); } // getAnimatorByID ()

    // TEXTURE
    @Override
    public Identifier getTextureByID(int value) { return InternalMetric.getTexture(EntityVariant.EnderPuffball, EntityTexture.byId(value)); } // getTextureByID ()

    // VARIANT
    @Override
    public String getVariant() {
        return this.getVariant(EntityVariant.EnderPuffball.getName());
    } // getVariant ()

    // STATS
    public float getAttributeRaw(EntityAttribute attribute) {
        return InternalMetric.getAttribute(EntityVariant.EnderPuffball, attribute);
    } // getAttributeRaw ()

    // -- Constructor --
    public MushroomGirlEnderPuffballEntity(EntityType<? extends TameableEntity> entityType, World world) {
        super(entityType, world);
    } // Constructor MushroomGirlEnderPuffballEntity ()
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, EntityData entityData, NbtCompound entityNbt) {
        this.setVariant(EntityVariant.EnderPuffball.getName());
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(VARIANT, EntityVariant.EnderPuffball.getName());
        this.dataTracker.startTracking(MODEL_ID, EntityModel.MushroomFat.getId());
        this.dataTracker.startTracking(BELLY, true);
    } // initDataTracker ()

} // Class MushroomGirlEnderPuffballEntity