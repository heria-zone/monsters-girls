    ...
    protected void handlePlanting (WorldAccess world, double x, double y, double z, Entity entity) {
        ...
        if (entity.isOnGround()) {
            ...
            if (world.isSpaceEmpty(new Box(blockPos))) {
                if ((world.getBlockState(new BlockPos((int)x, (int)y - 1, (int)z))).getBlock() == Blocks.END_STONE || (world.getBlockState(new BlockPos((int)x, (int)y - 1, (int)z))).getBlock() == MonstersGirlsBlocks.ENDER_MOSS) {
                    if (Math.random() < 5e-7) world.setBlockState(blockPos, MonstersGirlsBlocks.HUGE_ENDER_PUFFBALL.getDefaultState(), 3);
                    if (Math.random() < 0.0005) world.setBlockState(blockPos, MonstersGirlsBlocks.ENDER_PUFFBALL_MUSHROOM.getDefaultState(), 3);
                }
            }
        }
    } // handlePlanting ()
