...
public class MushroomCrimsonEntity extends InternalEntity implements GeoEntity  {
    ...
    public static DefaultAttributeContainer.Builder setAttributes() {
        return MobEntity.createMobAttributes()
                ...
                .add(EntityAttributes.GENERIC_ARMOR, InternalMetric.getAttribute(EntityVariant.MushroomCrimson, EntityAttribute.ARMOR))
                .add(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, InternalMetric.getAttribute(EntityVariant.MushroomCrimson, EntityAttribute.ARMOR_TOUGHNESS));
    } // setAttributes ()
    ...
    protected void handlePlanting (WorldAccess world, double x, double y, double z, Entity entity) {
        ...
        if (entity.isOnGround()) {
            ...
            if (world.isSpaceEmpty(new Box(blockPos))) {
                if ((world.getBlockState(new BlockPos((int)x, (int)y - 1, (int)z))).getBlock() == Blocks.CRIMSON_NYLIUM) {
                    if (Math.random() < 5e-7) world.setBlockState(blockPos, MonstersGirlsBlocks.HUGE_CRIMSON_FUNGUS.getDefaultState(), 3);
