...
public class MushroomInkCapEntity extends InternalEntity implements GeoEntity {
    ...
    public static DefaultAttributeContainer.Builder setAttributes() {
        return MobEntity.createMobAttributes()
                ...
                .add(EntityAttributes.GENERIC_ARMOR, InternalMetric.getAttribute(EntityVariant.FungusInkCap, EntityAttribute.ARMOR))
                .add(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, InternalMetric.getAttribute(EntityVariant.FungusInkCap, EntityAttribute.ARMOR_TOUGHNESS));
    } // setAttributes ()
        ...
        if (entity.isOnGround()) {
            ...
            if (world.isSpaceEmpty(new Box(blockPos))) {
                if ((world.getBlockState(new BlockPos((int)x, (int)y - 1, (int)z))).getBlock() == Blocks.GRASS_BLOCK) {
                    if (Math.random() < 5e-7) world.setBlockState(blockPos, MonstersGirlsBlocks.HUGE_INK_CAP_MUSHROOM.getDefaultState(), 3);
                    if (Math.random() < 0.0005) world.setBlockState(blockPos, MonstersGirlsBlocks.INK_CAP_MUSHROOM.getDefaultState(), 3);
                }
            }
        }
    } // handlePlanting ()
