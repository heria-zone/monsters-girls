...
public class MushroomMoltenEntity extends InternalEntity implements GeoEntity  {
    ...
    public static DefaultAttributeContainer.Builder setAttributes() {
        return MobEntity.createMobAttributes()
                .add(EntityAttributes.GENERIC_MAX_HEALTH, InternalMetric.getAttribute(EntityVariant.MushroomMolten, EntityAttribute.MAX_HEALTH))
    ...
    // -- Constructor --
    public MushroomMoltenEntity(EntityType<? extends TameableEntity> entityType, World world) {
        super(entityType, world);
        this.category = EntityCategory.Mushroom;
        this.variant = EntityVariant.MushroomMolten;
    } // Constructor MushroomMoltenEntity ()
    ...
    protected void handlePlanting (WorldAccess world, double x, double y, double z, Entity entity) {
        ...
        if (entity.isOnGround()) {
            ...
            if (world.isSpaceEmpty(new Box(blockPos))) {
                var canBlock = (world.getBlockState(new BlockPos((int)x, (int)y - 1, (int)z))).getBlock();
                if (canBlock == Blocks.BASALT || canBlock == Blocks.BLACKSTONE || canBlock == Blocks.MAGMA_BLOCK) {
                    if (Math.random() < 5e-7) world.setBlockState(blockPos, MonstersGirlsBlocks.HUGE_MOLTEN_FUNGUS.getDefaultState(), 3);
                    if (Math.random() < 0.0005) world.setBlockState(blockPos, MonstersGirlsBlocks.MOLTEN_FUNGUS.getDefaultState(), 3);
                }
            }
        }
    } // handlePlanting ()
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(VARIANT, EntityVariant.MushroomMolten.getName());
