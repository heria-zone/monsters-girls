...
public class InkCapPlantRightClickedProcedure {
	@SubscribeEvent
			...
			if ((world.getBlockState(new BlockPos(x, y, z))).getBlock() == MonstersAndGirlsModBlocks.INK_CAP_MUSHROOM.get()) {
				...
				if (Math.random() < 0.3) {
					if (world instanceof ServerLevel _serverworld) {
						StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("monsters_and_girls", "inkcap1"));
						if (template != null) {
				...
				if (Math.random() < 0.3) {
					if (world instanceof ServerLevel _serverworld) {
						StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("monsters_and_girls", "inkcap2"));
						if (template != null) {
							template.placeInWorld(_serverworld, new BlockPos(x - 2, y, z - 2), new BlockPos(x - 2, y, z - 2), new StructurePlaceSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false),
									_serverworld.random, 3);
						}
