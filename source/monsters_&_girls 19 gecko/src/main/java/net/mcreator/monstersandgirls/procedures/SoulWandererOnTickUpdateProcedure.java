...
public class SoulWandererOnTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.isOnGround()) {
			if (world.isEmptyBlock(new BlockPos(x, y, z))) {
				if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.SOUL_SAND) {
					if (Math.random() < 0.0005) {
						world.setBlock(new BlockPos(x, y, z), MonstersAndGirlsModBlocks.SOUL_WANDERER_MUSHROOM.get().defaultBlockState(), 3);
					}
				}
				if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.SOUL_SOIL) {
					if (Math.random() < 0.0005) {
						world.setBlock(new BlockPos(x, y, z), MonstersAndGirlsModBlocks.SOUL_WANDERER_MUSHROOM.get().defaultBlockState(), 3);
					}
				}
			...
			if (world.isEmptyBlock(new BlockPos(x, y, z - 1))) {
				if ((world.getBlockState(new BlockPos(x, y - 1, z - 1))).getBlock() == Blocks.SOUL_SAND) {
					if (Math.random() < 5e-7) {
						world.setBlock(new BlockPos(x, y, z - 1), MonstersAndGirlsModBlocks.HUGE_SOUL_WANDERER.get().defaultBlockState(), 3);
					}
				}
				if ((world.getBlockState(new BlockPos(x, y - 1, z - 1))).getBlock() == Blocks.SOUL_SOIL) {
					if (Math.random() < 5e-7) {
						world.setBlock(new BlockPos(x, y, z - 1), MonstersAndGirlsModBlocks.HUGE_SOUL_WANDERER.get().defaultBlockState(), 3);
					}
				}
		...
		if ((entity instanceof Player _plr ? _plr.getAbilities().getWalkingSpeed() : 0) == 0) {
			if (Math.random() < 0.0005) {
				if (entity instanceof SoulWandererGalEntity) {
					((SoulWandererGalEntity) entity).setAnimation("idle2");
				}
			}
			if (Math.random() < 0.0005) {
				if (entity instanceof SoulWandererGalEntity) {
					((SoulWandererGalEntity) entity).setAnimation("wave");
				}
			}
		}
		if ((entity instanceof Player _plr ? _plr.getAbilities().getWalkingSpeed() : 0) > 0) {
			if (entity instanceof SoulWandererGalEntity) {
				((SoulWandererGalEntity) entity).setAnimation("empty");
			}
		}
