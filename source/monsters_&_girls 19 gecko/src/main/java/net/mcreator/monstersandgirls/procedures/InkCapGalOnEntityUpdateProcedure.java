...
public class InkCapGalOnEntityUpdateProcedure {
			...
			if (world.isEmptyBlock(new BlockPos(x, y, z))) {
				if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.PODZOL) {
					if (Math.random() < 0.0005) {
						world.setBlock(new BlockPos(x, y, z), MonstersAndGirlsModBlocks.INK_CAP_MUSHROOM.get().defaultBlockState(), 3);
					}
				}
		...
		if (world.isEmptyBlock(new BlockPos(x, y, z - 1))) {
			if ((world.getBlockState(new BlockPos(x, y - 1, z - 1))).getBlock() == Blocks.PODZOL) {
				if (Math.random() < 0.000005) {
					world.setBlock(new BlockPos(x, y, z - 1), MonstersAndGirlsModBlocks.HUGE_INKY_CAP.get().defaultBlockState(), 3);
				}
			}
		...
		if ((entity instanceof Player _plr ? _plr.getAbilities().getWalkingSpeed() : 0) == 0) {
			if (Math.random() < 0.0005) {
				if (entity instanceof InkCapEntity) {
					((InkCapEntity) entity).setAnimation("idle2");
				}
			}
			if (Math.random() < 0.0005) {
				if (entity instanceof InkCapEntity) {
					((InkCapEntity) entity).setAnimation("wave");
				}
			}
		}
