...
public class RareWarpedJarDiesProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if (world instanceof ServerLevel _level) {
			Entity entityToSpawn = new WarpedMushroomGalBigSpotsEntity(MonstersAndGirlsModEntities.WARPED_MUSHROOM_GAL_BIG_SPOTS.get(), _level);
			entityToSpawn.moveTo(x, y, z, world.getRandom().nextFloat() * 360F, 0);
			if (entityToSpawn instanceof Mob _mobToSpawn)
				_mobToSpawn.finalizeSpawn(_level, world.getCurrentDifficultyAt(entityToSpawn.blockPosition()), MobSpawnType.MOB_SUMMONED, null, null);
			world.addFreshEntity(entityToSpawn);
		}
		if (((Entity) world.getEntitiesOfClass(WarpedMushroomGalBigSpotsEntity.class, AABB.ofSize(new Vec3(x, y, z), 1, 1, 1), e -> true).stream().sorted(new Object() {
