...
public class AlexgaricupdatetickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
			...
			if (world.isEmptyBlock(new BlockPos(x, y, z - 1))) {
				if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.PODZOL) {
					if (Math.random() < 0.000005) {
						world.setBlock(new BlockPos(x, y, z - 1), MonstersAndGirlsModBlocks.YELLOW_FLY_AGARIC_HUGE.get().defaultBlockState(), 3);
					}
				}
				if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.MYCELIUM) {
					if (Math.random() < 0.00005) {
		...
		if ((entity instanceof Player _plr ? _plr.getAbilities().getWalkingSpeed() : 0) == 0) {
			if (Math.random() < 0.0005) {
				if (entity instanceof FlyAlexgaricEntity) {
					((FlyAlexgaricEntity) entity).setAnimation("idle2");
				}
			}
			if (Math.random() < 0.0005) {
				if (entity instanceof FlyAlexgaricEntity) {
					((FlyAlexgaricEntity) entity).setAnimation("wave");
				}
			}
		}
		...
		if (!world.getEntitiesOfClass(Fox.class, AABB.ofSize(new Vec3(x, y, z), 4, 4, 4), e -> true).isEmpty()) {
			entity.startRiding(((Entity) world.getEntitiesOfClass(Fox.class, AABB.ofSize(new Vec3(x, y, z), 4, 4, 4), e -> true).stream().sorted(new Object() {
				Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
					return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
				}
			}.compareDistOf(x, y, z)).findFirst().orElse(null)));
			if (((Entity) world.getEntitiesOfClass(Fox.class, AABB.ofSize(new Vec3(x, y, z), 2, 2, 2), e -> true).stream().sorted(new Object() {
				Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
					return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
				}
