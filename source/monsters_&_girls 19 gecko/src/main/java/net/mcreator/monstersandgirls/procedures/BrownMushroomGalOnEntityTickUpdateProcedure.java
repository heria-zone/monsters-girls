...
public class BrownMushroomGalOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.isOnGround()) {
			if (world.isEmptyBlock(new BlockPos(x, y, z))) {
				if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.MYCELIUM) {
					...
					if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.PODZOL) {
						if (Math.random() < 0.0005) {
							world.setBlock(new BlockPos(x, y, z), Blocks.BROWN_MUSHROOM.defaultBlockState(), 3);
						}
					}
			...
			if (world.isEmptyBlock(new BlockPos(x, y, z - 1))) {
				if ((world.getBlockState(new BlockPos(x, y - 1, z - 1))).getBlock() == Blocks.MYCELIUM) {
					if (Math.random() < 0.000005) {
						world.setBlock(new BlockPos(x, y, z - 1), MonstersAndGirlsModBlocks.HUGE_BROWN_MUSHROOM.get().defaultBlockState(), 3);
					}
					...
					if ((world.getBlockState(new BlockPos(x, y - 1, z - 1))).getBlock() == Blocks.PODZOL) {
						if (Math.random() < 0.000005) {
							world.setBlock(new BlockPos(x, y, z - 1), MonstersAndGirlsModBlocks.HUGE_BROWN_MUSHROOM.get().defaultBlockState(), 3);
						}
					}
		...
		if ((entity instanceof Player _plr ? _plr.getAbilities().getWalkingSpeed() : 0) == 0) {
			if (Math.random() < 0.0005) {
				if (entity instanceof BrownMushroomGalEntity) {
					((BrownMushroomGalEntity) entity).setAnimation("belly");
				}
			}
			if (Math.random() < 0.0005) {
				if (entity instanceof BrownMushroomGalEntity) {
					((BrownMushroomGalEntity) entity).setAnimation("wave");
				}
			}
		}
		...
		if (!world.getEntitiesOfClass(Pig.class, AABB.ofSize(new Vec3(x, y, z), 3, 3, 3), e -> true).isEmpty()) {
			entity.startRiding(((Entity) world.getEntitiesOfClass(Pig.class, AABB.ofSize(new Vec3(x, y, z), 3, 3, 3), e -> true).stream().sorted(new Object() {
				Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
					return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
				}
			}.compareDistOf(x, y, z)).findFirst().orElse(null)));
