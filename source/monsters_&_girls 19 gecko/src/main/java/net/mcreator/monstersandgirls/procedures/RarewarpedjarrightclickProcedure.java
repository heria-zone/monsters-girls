...
public class RarewarpedjarrightclickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == MonstersAndGirlsModItems.RARE_WARPEDGIRLINA_JAR.get()) {
			if (world.isEmptyBlock(new BlockPos(x, y + 1, z))) {
				if (!(entity instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
					if (entity instanceof Player _player) {
						ItemStack _stktoremove = new ItemStack(MonstersAndGirlsModItems.RARE_WARPEDGIRLINA_JAR.get());
						_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
					}
				}
				MonstersAndGirlsMod.queueServerWork(1, () -> {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = new RareWarpedJarEntity(MonstersAndGirlsModEntities.RARE_WARPED_JAR.get(), _level);
