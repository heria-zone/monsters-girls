...
public class PeachRightClickedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		if (entity instanceof PeachEntity) {
			if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == MonstersAndGirlsModItems.SPECTRAL_CAKE.get()) {
				...
				if (entity.isPassenger()) {
					if (entity instanceof PeachEntity) {
						((PeachEntity) entity).setAnimation("eat_sit");
					}
				} else if (!entity.isPassenger()) {
					if (entity instanceof PeachEntity) {
						((PeachEntity) entity).setAnimation("spook.eat");
					}
				}
