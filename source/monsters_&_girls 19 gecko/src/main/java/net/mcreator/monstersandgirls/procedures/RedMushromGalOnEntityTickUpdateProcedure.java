...
public class RedMushromGalOnEntityTickUpdateProcedure {
			...
			if (world.isEmptyBlock(new BlockPos(x, y, z - 1))) {
				if ((world.getBlockState(new BlockPos(x, y - 1, z - 1))).getBlock() == Blocks.PODZOL) {
					if (Math.random() < 0.000005) {
						world.setBlock(new BlockPos(x, y, z - 1), MonstersAndGirlsModBlocks.HUGE_TOADSTOOL.get().defaultBlockState(), 3);
					}
				}
		...
		if ((entity instanceof Player _plr ? _plr.getAbilities().getWalkingSpeed() : 0) == 0) {
			if (Math.random() < 0.0005) {
				if (entity instanceof RedMushromGalEntity) {
					((RedMushromGalEntity) entity).setAnimation("idle2");
				}
			}
			if (Math.random() < 0.0005) {
				if (entity instanceof RedMushromGalEntity) {
					((RedMushromGalEntity) entity).setAnimation("wave");
				}
			}
		}
