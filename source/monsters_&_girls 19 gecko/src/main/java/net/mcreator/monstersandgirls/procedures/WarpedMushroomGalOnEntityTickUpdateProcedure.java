...
public class WarpedMushroomGalOnEntityTickUpdateProcedure {
		...
		if (entity.isOnGround()) {
			...
			if (world.isEmptyBlock(new BlockPos(x, y, z - 1))) {
				if ((world.getBlockState(new BlockPos(x, y - 1, z - 1))).getBlock() == Blocks.WARPED_NYLIUM) {
					if (Math.random() < 5e-7) {
						world.setBlock(new BlockPos(x, y, z - 1), MonstersAndGirlsModBlocks.HUGE_WARPED_FUNGUS.get().defaultBlockState(), 3);
					}
				}
			}
		}
		if ((entity instanceof Player _plr ? _plr.getAbilities().getWalkingSpeed() : 0) == 0) {
			if (Math.random() < 0.0005) {
				if (entity instanceof WarpedMushroomGalEntity) {
					((WarpedMushroomGalEntity) entity).setAnimation("idle2");
				}
			}
			if (Math.random() < 0.0005) {
				if (entity instanceof WarpedMushroomGalEntity) {
					((WarpedMushroomGalEntity) entity).setAnimation("wave");
				}
			}
		}
