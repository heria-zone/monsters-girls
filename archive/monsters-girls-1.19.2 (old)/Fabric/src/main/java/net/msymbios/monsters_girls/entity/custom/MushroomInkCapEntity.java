        ...
        if (entity.isOnGround()) {
            ...
            if (world.isSpaceEmpty(new Box(blockPos))) {
                if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.GRASS_BLOCK) {
                    if (Math.random() < 5e-7) world.setBlockState(blockPos, ModBlocks.HUGE_INK_CAP_MUSHROOM.getDefaultState(), 3);
                    if (Math.random() < 0.0005) world.setBlockState(blockPos, ModBlocks.INK_CAP_MUSHROOM.getDefaultState(), 3);
                }
            }
        }
    } // handlePlanting ()
